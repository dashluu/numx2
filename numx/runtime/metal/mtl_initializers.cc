...
namespace nx::runtime::metal {
    void MTLRuntime::run_full_kernel(Op *op, usize constant) {
        ...
        runner.encode_mtl_buffer(&mtl_constant, sizeof(mtl_usize));
        runner.encode_array_buffer(descriptor);
    ...
    void MTLRuntime::run_arange_kernel(Op *op, usize start, usize step) {
        ...
        runner.encode_mtl_buffer(&mtl_start, sizeof(mtl_usize));
        runner.encode_mtl_buffer(&mtl_step, sizeof(mtl_usize));
        runner.encode_array_buffer(descriptor);
    ...
    void MTLRuntime::run_uniform_kernel(Op *op, std::uint64_t key, usize low, usize high) {
        ...
        runner.encode_mtl_buffer(&key, sizeof(std::uint64_t));
        runner.encode_mtl_buffer(&mtl_low, sizeof(mtl_usize));
        runner.encode_mtl_buffer(&mtl_high, sizeof(mtl_usize));
        ...
        runner.dispatch_threads(num_quad_words, std::min(num_quad_words, s_threadgroup_size));
        runner.run();
        pool->release();
    }
} // namespace nx::runtime::metal
